...
from .zoosync_core import ZooQueue
...
class ZooPool:
    def __init__(self, num_workers=None, buffer_size_mb=10):
        ...
        self.task_q_name = f"zp_task_{self.id}"
        self.result_q_name = f"zp_res_{self.id}"
        self.buffer_size_mb = buffer_size_mb
        
        # Initialize Queues
        self.task_queue = ZooQueue(self.task_q_name, buffer_size_mb)
        self.result_queue = ZooQueue(self.result_q_name, buffer_size_mb)
        ...
        for _ in range(num_workers):
            p = multiprocessing.Process(
                ...
                args=(self.task_q_name, self.result_q_name, self.buffer_size_mb)
            )
            p.start()
    ...
    def _result_listener(self):
        while not self._shutdown:
            try:
                res_bytes = self.result_queue.get_bytes()
    ...
    def submit(self, func, *args, **kwargs):
        ...
        self.task_queue.put_bytes(pickle.dumps(payload))
    ...
    def shutdown(self, wait=True):
        ...
        for _ in range(self.num_workers):
            try:
                self.task_queue.put_bytes(pickle.dumps((None, None, None, None)))
            except:
                pass
        ...
        # Cleanup SHM
        try:
            ZooQueue.unlink(self.task_q_name)
            ZooQueue.unlink(self.result_q_name)
        except:
            pass
